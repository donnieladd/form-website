...
| 14 | Intelligent-intake architecture | 🟡 | Deterministic `routeFor()` table, no AI. Behind `CONTACT_ROUTING` env flag. **Split 2026-08-13:** the visible taxonomy at `contact.html:70` ships with the nav flip so the two never disagree; the routing logic follows separately. | Owner | `contact.html:70` (with nav flip) · `api/contact.mjs` + `tests/contact-api.test.js` (later) |

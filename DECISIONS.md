...
| — | Public services architecture | **Superseding directive, 2026-08-13.** Methodology / capability / business / engagement model are separated. Solutions Architecture = the method, not a service door. Six capability doors: advisory & transformation · solutions & intelligence · digital services · creative & marketing · live experience · support. Continuum = engagement model, not a practice, not under Support. The *number* of doors is not a brand promise — "four practices" is retired and "five"/"six practices" are banned too. | 2026-08-13 |
| — | Creative & Experience split | Retired as one practice. **Creative & Marketing** (→ form. creative & marketing) and **Live Experience** (→ form. experience) are different buying categories and different businesses. `creative-experience.html` retired, 301 → `/creative-marketing.html` — the larger share of that page's intent; the live half is one click on from there. | 2026-08-13 |
| — | Managed Services → Support | Retired as a category name. **Support** covers three territories — human (EA/VA/admin), systems (platform, technical, AI operations), operational (recurring execution) — and is not reduced to help desk or to VA work. `managed-services.html` retired, 301 → `/support.html`. | 2026-08-13 |
| — | No public form. AI business | AI is **not** split into its own public business. AI strategy, enablement, governance and intelligence architecture stay in **Solutions & Intelligence**; **form. digital** engineers the AI-enabled products they run inside. `digital.html` no longer reads as "websites and apps". | 2026-08-13 |
| — | Canonical email aliases | `hello@` `advisory@` `solutions@` `digital@` `creative@` `experience@` `support@` `continuum@`, plus `ministry@` for the ministry workflow. Short forms only — no `digitalservices@` / `liveexperience@`. `managed@` retired. Pinned by a test that fails if `routeFor` ever returns an alias outside this set. | 2026-08-13 |
...
| 14 | Intelligent-intake architecture | 🟡 | Deterministic `routeFor()` table SHIPPED behind `CONTACT_ROUTING` (off). Retaxonomised 2026-08-13. **Aliases created 2026-08-13** (owner): advisory@ · solutions@ · digital@ · creative@ · experience@ · support@ · continuum@ · ministry@. `managed@` retired — not recreated. **Remaining: send a live test to each and confirm receipt, then `vercel env add CONTACT_ROUTING production` = `on`.** Creation is not proof of delivery — a mistyped or unrouted alias fails silently, and the audit row in the inquiry email is what confirms it end to end. Kill switch: `vercel env rm CONTACT_ROUTING`. | Owner | live test + env var (manual) — code is done |

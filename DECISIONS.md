...
| 14 | Intelligent-intake architecture | 🟡 | Deterministic `routeFor()` table SHIPPED behind `CONTACT_ROUTING` (off). Retaxonomised 2026-08-13. To turn on: create **advisory@ · solutions@ · digital@ · creative@ · experience@ · support@ · continuum@ · ministry@**, send a live test to each, then `vercel env add CONTACT_ROUTING production` = `on`. Kill switch: `vercel env rm CONTACT_ROUTING`. **`digital@`, `experience@` and `support@` are new as of 2026-08-13 and have not been live-tested.** `managed@` is retired — do not create it. | Owner | aliases + env var (manual) — code is done |
